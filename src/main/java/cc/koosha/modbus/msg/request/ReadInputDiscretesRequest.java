...
import cc.koosha.modbus.msg.response.ReadInputDiscretesResponse;
import cc.koosha.modbus.procimg.DigitalIn;
 ...
 */
public final class ReadInputDiscretesRequest extends AbstractModbusRequest {
     ...
     */
    public ReadInputDiscretesRequest(int unitId, int ref, int count) {
        super();
    ...
    public ModbusResponse createResponse(ProcessImage pi) {
        ...
        try {
            dins = pi.getDigitalInRange(getReference(), getBitCount());
        }
        ...
        for (int i = 0; i < dins.size(); i++) {
            response.setDiscreteStatus(i, dins.get(i).isSet());
        }
