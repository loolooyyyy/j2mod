...
import cc.koosha.modbus.msg.response.ReadFileRecordResponse;
import cc.koosha.modbus.msg.response.ReadFileRecordResponse.RecordResponse;
import cc.koosha.modbus.procimg.*;
 ...
 */
public final class ReadFileRecordRequest extends AbstractModbusRequest {
    ...
    public ModbusResponse createResponse(ProcessImage pi) {
        ...
        try {
            for (int i = 0; i < getRequestCount(); i++) {
                ...
                if (recordRequest.getFileNumber() < 0 || recordRequest.getFileNumber() >= pi.getFileCount()) {
                    return createExceptionResponse(Modbus.ILLEGAL_ADDRESS_EXCEPTION);
                }

                File file = pi.getFileByNumber(recordRequest.getFileNumber());
