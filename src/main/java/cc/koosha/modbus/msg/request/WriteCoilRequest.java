...
import cc.koosha.modbus.msg.response.WriteCoilResponse;
...
/**
 * Class implementing a <tt>WriteCoilRequest</tt>. The implementation directly correlates with the class 0 function
 * <i>write coil (FC 5)</i>. It encapsulates the corresponding request message.
 *
 * @author Dieter Wimberger
 * @author Steve O'Hara (4NG)
 * @version 2.0 (March 2016)
 */
public final class WriteCoilRequest extends AbstractModbusRequest {
    ...
    /**
     * Constructs a new <tt>WriteCoilRequest</tt> instance with a given reference and state to be written.
     *
     ...
     */
    public WriteCoilRequest(int unitId, int ref, boolean b) {
        super();
        setUnitID(unitId);
    ...
    /**
     * Returns the reference of the register of the coil that should be written to with this <tt>ReadCoilsRequest</tt>.
     *
    ...
    /**
     * Sets the reference of the register of the coil that should be written to with this <tt>ReadCoilsRequest</tt>.
     * <p>
     *
    ...
    public void writeData(DataOutput dout) throws IOException {
        ...
        if (coil) {
            dout.write(Modbus.coilOnBytes(), 0, 2);
        }
        else {
            dout.write(Modbus.coilOffBytes(), 0, 2);
        }
    }
    ...
    public byte[] getMessage() {
        ...
        if (coil) {
            result[2] = Modbus.coilOnBytes()[0];
            result[3] = Modbus.coilOnBytes()[1];
        }
        else {
            result[2] = Modbus.coilOffBytes()[0];
            result[3] = Modbus.coilOffBytes()[1];
        }
        return result;
    }
}